...
# Importar conversor mejorado de IF/EVALUATE
try:
    from enhanced_if_evaluate_converter import IfEvaluateConverterFactory
    ENHANCED_IF_AVAILABLE = True
except ImportError:
    ENHANCED_IF_AVAILABLE = False
    print("⚠️  Conversor mejorado IF/EVALUATE no disponible, usando versión estándar")

# Importar conversor de nombres cualificados
try:
    from cobol_qualified_names_converter import QualifiedNameConverterFactory, QualifiedNameStrategy
    QUALIFIED_NAMES_AVAILABLE = True
except ImportError:
    QUALIFIED_NAMES_AVAILABLE = False
    print("⚠️  Conversor de nombres cualificados no disponible, usando versión estándar")

# Importar conversor de operaciones de archivos
try:
    from cobol_file_operations_converter import FileOperationConverterFactory
    FILE_OPERATIONS_AVAILABLE = True
except ImportError:
    FILE_OPERATIONS_AVAILABLE = False
    print("⚠️  Conversor de operaciones de archivos no disponible, usando versión estándar")

# Importar conversor de sentencias SET
try:
    from cobol_set_statement_converter import SetStatementConverterFactory
    SET_STATEMENTS_AVAILABLE = True
except ImportError:
    SET_STATEMENTS_AVAILABLE = False
    print("⚠️  Conversor de sentencias SET no disponible, usando versión estándar")
...
class GenericNodeProcessor(AbstractNodeProcessor):
    ...
    def process(self, node: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "PERFORM" in text_upper:
            ...
            op = "ELSE"
        elif "EVALUATE" in text_upper and text_upper.startswith("EVALUATE"):
            op = "EVALUATE"
        elif "END-EVALUATE" in text_upper:
            op = "END_EVALUATE"
        elif "WHEN" in text_upper and text_upper.startswith("WHEN"):
            op = "WHEN"
        elif text_upper.startswith("OPEN "):
            op = "OPEN"
        elif text_upper.startswith("CLOSE "):
            op = "CLOSE"
        elif text_upper.startswith("READ "):
            op = "READ"
        elif text_upper.startswith("WRITE "):
            op = "WRITE"
        elif text_upper.startswith("REWRITE "):
            op = "REWRITE"
        elif text_upper.startswith("DELETE "):
            op = "DELETE"
        elif text_upper.startswith("START "):
            op = "START"
        elif text_upper.startswith("SET "):
            op = "SET"
        else:
            op = "STATEMENT"
...
class IRToSQLConverter:
    ...
    def __init__(self):
        ...
        # Inicializar conversor mejorado de IF/EVALUATE si está disponible
        if ENHANCED_IF_AVAILABLE:
            self.enhanced_if_converter = IfEvaluateConverterFactory()
            print("✅ Conversor mejorado IF/EVALUATE cargado")
        else:
            self.enhanced_if_converter = None

        # Inicializar conversor de nombres cualificados si está disponible
        if QUALIFIED_NAMES_AVAILABLE:
            self.qualified_names_converter = QualifiedNameConverterFactory()
            print("✅ Conversor de nombres cualificados cargado")
        else:
            self.qualified_names_converter = None

        # Inicializar conversor de operaciones de archivos si está disponible
        if FILE_OPERATIONS_AVAILABLE:
            self.file_operations_converter = FileOperationConverterFactory()
            print("✅ Conversor de operaciones de archivos cargado")
        else:
            self.file_operations_converter = None

        # Inicializar conversor de sentencias SET si está disponible
        if SET_STATEMENTS_AVAILABLE:
            self.set_statements_converter = SetStatementConverterFactory()
            print("✅ Conversor de sentencias SET cargado")
        else:
            self.set_statements_converter = None
...
"""
  ...
  -- DBMS_OUTPUT.PUT_LINE({raw_content.replace('DISPLAY', '').strip()}); -- (DISPLAY statement)"""
                            ...
                            # Aplicar conversión completa de EVALUATE usando conversor mejorado
                            if self.enhanced_if_converter and self.enhanced_if_converter.get_converter(raw_content):
                                try:
                                    converted_evaluate = self.enhanced_if_converter.convert_statement(raw_content, stmt.get("details", {}))
                                    procedures_section += converted_evaluate
                                    print("✅ EVALUATE convertido con conversor mejorado")
                                except Exception as e:
                                    print(f"⚠️  Error convirtiendo EVALUATE: {e}")
                                    procedures_section += f"""
          -- GAP -- {raw_content.strip()} -- (EVALUATE statement - conversion error)"""
                                    gap_count += 1
                            else:
                                procedures_section += f"""
          -- GAP -- {raw_content.strip()} -- (EVALUATE statement)"""
                                gap_count += 1
                        elif op in ["OPEN", "CLOSE", "READ", "WRITE", "REWRITE", "DELETE", "START"]:
                            # Aplicar conversión de operaciones de archivos usando conversor especializado
                            if self.file_operations_converter:
                                try:
                                    converted_file_op = self.file_operations_converter.convert_file_operation(raw_content.strip())
                                    procedures_section += converted_file_op
                                    print(f"✅ {op} convertido con conversor de archivos")
                                except Exception as e:
                                    print(f"⚠️  Error convirtiendo {op}: {e}")
                                    procedures_section += f"""
          -- GAP -- {raw_content.strip()} -- ({op} file operation - conversion error)"""
                                    gap_count += 1
                            else:
                                procedures_section += f"""
          -- GAP -- {raw_content.strip()} -- ({op} file operation)"""
                                gap_count += 1
                        elif op == "INITIALIZE":
                            procedures_section += f"""
  ...
  -- GAP -- {raw_content.strip()} -- ({op} statement)"""
    ...
    def _convert_move_simple(self, move_info: Dict[str, Any], raw_content: str) -> str:
        ...
        ENHANCED: Incluye conversión de nombres cualificados
        """
        source = move_info["source"]
        target = move_info["target"]

        # Aplicar conversión de nombres cualificados primero
        source_with_qualified = self._convert_qualified_names(source)
        target_with_qualified = self._convert_qualified_names(target)

        # Limpiar identificadores después de la conversión cualificada
        source_clean = self._clean_cobol_identifier(source_with_qualified)
        target_clean = self._clean_cobol_identifier(target_with_qualified)
  ...
  -- GAP -- {raw_content} -- (MOVE statement - generic)"""
    ...
    def _convert_qualified_names(self, expression: str, context: Dict[str, Any] = None) -> str:
        """
        Convertir nombres cualificados COBOL a PL/SQL usando conversor especializado
        ENHANCED: Utiliza el conversor de nombres cualificados si está disponible
        """
        if not expression:
            return expression

        # Usar conversor especializado si está disponible
        if self.qualified_names_converter:
            try:
                # Usar estrategia RECORDS por defecto (similar al archivo manual)
                converted = self.qualified_names_converter.convert_statement_with_qualified_names(
                    expression,
                ...
                print(f"⚠️  Error en conversor de nombres cualificados: {e}")

        # Fallback: conversión básica
        return self._convert_qualified_names_basic(expression)

    def _convert_qualified_names_basic(self, expression: str) -> str:
        """
        Conversión básica de nombres cualificados (fallback)
        Patrón básico: VARIABLE OF GROUP -> group.variable
        """
        import re

        # Patrón para nombres cualificados: VARIABLE OF GROUP OF SUPERGROUP
        triple_pattern = r'([A-Z][\w\-]*)\s+OF\s+([A-Z][\w\-]*)\s+OF\s+([A-Z][\w\-]*)'
        expression = re.sub(triple_pattern,
                           lambda m: f"{self._clean_identifier_basic(m.group(3))}.{self._clean_identifier_basic(m.group(2))}.{self._clean_identifier_basic(m.group(1))}",
                           expression, flags=re.IGNORECASE)

        # Patrón para nombres cualificados: VARIABLE OF GROUP
        double_pattern = r'([A-Z][\w\-]*)\s+OF\s+([A-Z][\w\-]*)'
        expression = re.sub(double_pattern,
                           lambda m: f"{self._clean_identifier_basic(m.group(2))}.{self._clean_identifier_basic(m.group(1))}",
                           expression, flags=re.IGNORECASE)

        # Patrón para nombres cualificados con IN: VARIABLE IN GROUP
        in_pattern = r'([A-Z][\w\-]*)\s+IN\s+([A-Z][\w\-]*)'
        expression = re.sub(in_pattern,
                           lambda m: f"{self._clean_identifier_basic(m.group(2))}.{self._clean_identifier_basic(m.group(1))}",
                           expression, flags=re.IGNORECASE)

        return expression

    def _clean_identifier_basic(self, identifier: str) -> str:
        """Limpiar identificador básico para fallback"""
        if not identifier:
            return identifier
    ...
    def _convert_if_statement(self, stmt: Dict[str, Any]) -> str:
        ...
        ENHANCED: Usa conversor mejorado si está disponible
        """
        raw_content = stmt.get("raw", "").strip()
        details = stmt.get("details", {})
        ...
        # Intentar conversor mejorado primero si está disponible
        if self.enhanced_if_converter and self.enhanced_if_converter.get_converter(raw_content):
            try:
                enhanced_result = self.enhanced_if_converter.convert_statement(raw_content, details)
                if enhanced_result and not enhanced_result.startswith("  -- GAP"):
                    print("✅ Usando conversor mejorado IF/EVALUATE")
                    return enhanced_result
            except Exception as e:
                print(f"⚠️  Error en conversor mejorado, usando versión estándar: {e}")

        # Fallback a conversor estándar
  ...
  -- GAP -- {raw_content} -- (IF statement - generic)"""
    ...
    def _convert_cobol_condition_to_plsql(self, condition: str) -> str:
        ...
        # Aplicar conversión de nombres cualificados primero
        converted = self._convert_qualified_names(condition)
...
-- ============================================="""
    ...
    def _convert_set_statement(self, stmt: Dict[str, Any]) -> str:
        ...
        Convertir statement SET a PL/SQL siguiendo equivalencias del archivo de referencia
        Principio Single Responsibility: Solo maneja conversión de SET
        ENHANCED: Usa conversor especializado si está disponible
        """
        raw_content = stmt.get("raw", "").strip()

        print(f"🔄 Convirtiendo SET: {raw_content}")

        # Intentar conversor especializado primero si está disponible
        if hasattr(self, 'set_statements_converter') and self.set_statements_converter:
            try:
                converted_result = self.set_statements_converter.convert_set_statement(raw_content)
                if converted_result and not converted_result.startswith("  -- GAP"):
                    print("✅ Usando conversor especializado SET")
                    return converted_result
            except Exception as e:
                print(f"⚠️  Error en conversor especializado SET, usando versión estándar: {e}")

        # Fallback a conversión básica
        return self._convert_set_statement_basic(raw_content)

    def _convert_set_statement_basic(self, raw_content: str) -> str:
        """
        Conversión básica de sentencias SET (fallback)
        Basado en patrones comunes del archivo manual
        """
        import re

        # Patrón básico: SET variable TO TRUE/FALSE
        boolean_pattern = r'SET\s+([A-Z][\w\-]*)\s+TO\s+(TRUE|FALSE)'
        match = re.search(boolean_pattern, raw_content, re.IGNORECASE)
        if match:
            variable = match.group(1).replace('-', '_').upper()
            value = match.group(2).upper()
            return f"\n    {variable} := {value};"

        # Patrón básico: SET index TO value
        index_pattern = r'SET\s+([A-Z][\w\-]*)\s+TO\s+([0-9]+)'
        match = re.search(index_pattern, raw_content, re.IGNORECASE)
        if match:
            index = match.group(1).replace('-', '_').upper()
            value = match.group(2)
            return f"\n    {index} := {value};"

        # Patrón básico: SET index UP/DOWN BY increment
        increment_pattern = r'SET\s+([A-Z][\w\-]*)\s+(UP|DOWN)\s+BY\s+([0-9]+)'
        match = re.search(increment_pattern, raw_content, re.IGNORECASE)
        if match:
            index = match.group(1).replace('-', '_').upper()
            direction = match.group(2).upper()
            increment = match.group(3)
            operator = '+' if direction == 'UP' else '-'
            return f"\n    {index} := {index} {operator} {increment};"

        # No reconocido - generar GAP
        return f"\n  -- GAP -- {raw_content} -- (SET statement - basic fallback)"
