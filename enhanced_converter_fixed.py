#!/usr/bin/env python3
"""
Convertidor COBOL a PL/SQL mejorado y robusto para casos complejos
"""
import sys
import os
import json
...
def _clean_expression(expr: str) -> str:
    ...
    # Manejar variables truncadas: F -> FOR_PAGO, I -> IND_TIP_CONTA, C -> COD_TIP_ORIENTACION, V -> VARIABLE
    expr = re.sub(r'\bF\b', 'FOR_PAGO', expr, flags=re.IGNORECASE)
    expr = re.sub(r'\bI\b', 'IND_TIP_CONTA', expr, flags=re.IGNORECASE)
    expr = re.sub(r'\bC\b', 'COD_TIP_ORIENTACION', expr, flags=re.IGNORECASE)
    expr = re.sub(r'\bV\b', 'VARIABLE', expr, flags=re.IGNORECASE)
    ...
    while i < len(lines):
        try:
            ...
            if m:
                src = m.group(1).strip()
                dst = m.group(2).strip()
                # Limpiar espacios extra en el destino
            ...
            if m:
                ...
                if condition.upper().endswith('OR'):
                    ...
                    if i < len(lines):
                        ...
                        condition += ' ' + next_line
                        # Si la siguiente línea también termina con OR, buscar otra línea más
                        if next_line.upper().endswith('OR'):
                            i += 1
                            if i < len(lines):
                                next_line2 = lines[i].strip()
                                next_line2 = re.sub(r'\s+THEN\s*$', '', next_line2, flags=re.IGNORECASE)
                                condition += ' ' + next_line2
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python enhanced_converter_fixed.py <archivo.cob>")
        sys.exit(1)

    cob_path = sys.argv[1]
    
    try:
        print(f"🚀 Convertidor COBOL a PL/SQL Mejorado y Robusto")
        ...
        with open(rep_path, "w", encoding="utf-8") as f:
            json.dump({"program": ir["program"], "coverage": coverage, "method": "ENHANCED_ROBUST"}, f, indent=2)
