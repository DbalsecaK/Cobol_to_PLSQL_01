...
authorParagraph
    : AUTHOR DOT_FS  (IDENTIFIER DOT_FS)?
    ;
...
dateWrittenParagraph
    : DATE_WRITTEN DOT_FS (IDENTIFIER DOT_FS | NONNUMERICLITERAL DOT_FS | COMMENTENTRYLINE)?
    ;
...
dataDescriptionEntryExecSql
    : EXEC_SQL_BLOCK DOT_FS #execSqlIncludeOrDeclare
    | EXECSQLLINE+ DOT_FS #execSqlLine
    ;
...
execSqlStatement
    : EXEC_SQL_BLOCK   #execSqlOther
    | EXECSQLLINE+      #execSqlLineOther
    ;
...
stringStatement
    : STRING (stringSendingPhrase+ | stringSendingList) stringIntoPhrase stringWithPointerPhrase? onOverflowPhrase? notOnOverflowPhrase? END_STRING?
    ;
...
expression
    : expression ASTERISKCHAR expression
    | expression ('+'|'-'|'/') expression
    | qualifiedDataName
    | INTEGERLITERAL
    | '(' expression ')'
