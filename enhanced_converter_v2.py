#!/usr/bin/env python3
"""
Convertidor COBOL a PL/SQL mejorado v2 - Parsing correcto
"""
import sys
import os
import json
...
def parse_cobol_to_ir_enhanced(file_path: str) -> Dict[str, Any]:
    ...
    print(f"🔍 Parseando con lógica mejorada v2: {file_path}")
    ...
    try:
        ...
        print(f"⚠️  Error extrayendo variables: {e}")
    ...
    try:
        ...
        statements = []
        if proc_section:
            statements = _extract_statements_enhanced_v2(proc_section.group(1))
        else:
...
def _extract_statements_enhanced_v2(proc_text: str) -> List[Dict[str, Any]]:
    """Extraer sentencias con lógica mejorada v2 - Parsing correcto"""
    ...
    while i < len(lines):
        try:
            ...
            # MOVE statement - regex mejorado para capturar todo el destino
            m = re.search(r'^MOVE\s+(.+?)\s+TO\s+(.+?)(?:\.|$)', ln, re.IGNORECASE)
            if m:
                src = m.group(1).strip()
                dst = m.group(2).strip()
                # Limpiar espacios extra pero mantener la estructura completa
            ...
            if m:
                ...
                if condition.upper().endswith('OR'):
                    ...
                    if i < len(lines):
                        ...
                        # Remover THEN si está presente
                        next_line = re.sub(r'\s+THEN\s*$', '', next_line, flags=re.IGNORECASE)
                        condition += ' ' + next_line
                ...
                while i < len(lines) and not re.search(r'^ELSE', lines[i], re.IGNORECASE) and not re.search(r'^END-IF', lines[i], re.IGNORECASE):
                    ...
                    if m:
                        ...
                        dst = m.group(2).strip()
                        dst = re.sub(r'\s+', ' ', dst).strip()
                        then_stmts.append({"op":"MOVE", "src": src, "dst": dst, "raw": then_ln})
                    # DISPLAY statement
                    ...
                    # IF anidado
                        ...
                        nested_if = _parse_nested_if_v2(lines, i)
                ...
                if i < len(lines) and re.search(r'^ELSE', lines[i], re.IGNORECASE):
                    ...
                    while i < len(lines) and not re.search(r'^END-IF', lines[i], re.IGNORECASE):
                        ...
                        if m:
                            ...
                            dst = m.group(2).strip()
                            dst = re.sub(r'\s+', ' ', dst).strip()
                            else_stmts.append({"op":"MOVE", "src": src, "dst": dst, "raw": else_ln})
                        # DISPLAY statement
            ...
            # Si no se reconoce, marcar como GAP
            stmts.append({"op":"GAP", "raw": ln})
            i += 1
...
def _parse_nested_if_v2(lines: List[str], start_index: int) -> Dict[str, Any]:
    """Parsear IF anidado recursivamente v2"""
    try:
...
def apply_rule(stmt: Dict[str, Any]) -> str:
    ...
    if op == "MOVE":
        ...
        # Procesar sentencias THEN con indentación
        then_code = ""
        for t in then_stmts:
            rule_result = apply_rule(t)
            # Agregar indentación extra para sentencias dentro del IF
            indented_lines = "\n".join([f"    {line}" for line in rule_result.split("\n")])
            then_code += indented_lines + "\n"
        
        # Procesar sentencias ELSE con indentación
        else_code = ""
        for t in else_stmts:
            rule_result = apply_rule(t)
            # Agregar indentación extra para sentencias dentro del ELSE
            indented_lines = "\n".join([f"    {line}" for line in rule_result.split("\n")])
            else_code += indented_lines + "\n"
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python enhanced_converter_v2.py <archivo.cob>")
        sys.exit(1)

    cob_path = sys.argv[1]
    
    try:
        print(f"🚀 Convertidor COBOL a PL/SQL Mejorado v2 - Parsing Correcto")
        ...
        ir_file = os.path.join(out_dir, f"{ir['program']}_v2_ir.json")
        ...
        pkg_path = os.path.join(out_dir, f"{ir['program']}_v2.sql")
        
        with open(pkg_path, "w", encoding="utf-8") as f:
            f.write(plsql)

        rep_path = os.path.join(out_dir, f"{ir['program']}_v2_report.json")
        with open(rep_path, "w", encoding="utf-8") as f:
            json.dump({"program": ir["program"], "coverage": coverage, "method": "ENHANCED_V2"}, f, indent=2)
